...
class LennardJones():
    def __init__(self, M_w):
        ...
        self.rhol_star_params = yfile["correlation_parameters"]
        ["Lofti"]["rho_L_star_params"]
        self.rhov_star_params = yfile["correlation_parameters"]
        ["Lofti"]["rho_v_star_params"]
        self.Psat_star_params = yfile["correlation_parameters"]
        ["Lofti"]["P_v_star_params"]
        self.deltaHv_star_params = yfile["correlation_parameters"]
        ["Lofti"]["HVP_star_params"]

        self.rhol_star_params = [
            self.rhoc_star,
            self.T_c_star,
            self.rhol_star_params[0],
            self.rhol_star_params[1],
            self.rhol_star_params[2]]
        self.rhov_star_params = [
            self.rhoc_star,
            self.T_c_star,
            self.rhov_star_params[0],
            self.rhov_star_params[1],
            self.rhov_star_params[2]]

    def rhol_star_hat(self, T_star):
        b = self.rhol_star_params
        tau = np.ones(len(T_star)) * b[1] - T_star  # T_c_star - T_star
        rhol_star = b[0] + b[2] * tau**(1. / 3) +
        b[3] * tau + b[4] * tau**(3. / 2)
        return rhol_star
    ...
    def Psat_hat_LJ(self, T, eps, sig):
        ...
        Psat = Psat_star * eps / sig**3 * k_B * m3_to_nm3 * J_per_m3_to_kPA
        # [kPa]
        return Psat
    ...
    def B2_hat_LJ(self, T, eps, sig):
        ...
        for i, t_star in enumerate(T_star):
            addend = pow(2, (2 * n + 1.) / 2) *
            pow(1. / t_star, (2 * n + 1.) / 4) *
            sp.special.gamma((2 * n - 1.) / 4) /
            (4 * sp.misc.factorial(n))
            # B2_star[i] = addend.sum()
            # This is the standard approach but sometimes 'nan' results
            B2_star[i] = np.nansum(addend)
            # This can handle even 'nan' results
    ...
    def calc_sig_rhoc(self, rhoc):
        sig_rhoc = (self.rhoc_star / rhoc * self.M_w / N_A *
                    m3_to_nm3 * gm_to_kg)**(1. / 3)  # [nm]
        return sig_rhoc
