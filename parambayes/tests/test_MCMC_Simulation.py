...
class TestInit(unittest.TestCase):
    ...
    def test_trange(self):
        ...
        self.assertRaises(ValueError,
                          ...
                          simulation_params['number_data_points'],
                          simulation_params['steps'])
        self.assertRaises(ValueError, MCMC_Simulation,
                          simulation_params['compound'],
                          [0.5, 2], simulation_params['properties'],
                          simulation_params['number_data_points'],
                          simulation_params['steps'])
        self.assertRaises(ValueError,
                          MCMC_Simulation,
...
class TestCalcPosterior(unittest.TestCase):
    def setup():
        mcmc_simulator = \
            MCMC_Simulation(simulation_params['compound'],
                            simulation_params['trange'],
                            simulation_params['properties'],
                            simulation_params['number_data_points'],
                            simulation_params['steps'])
    ...
    def test_nan_handle(self):
        ...
        self.assertWarns(UserWarning, mcmc_simulator.calc_posterior,
...
class TestSetInitialState(unittest.TestCase):
    ...
    def test_inputs(self):
        ...
        self.assertRaises(IndexError, mcmc_simulator.set_initial_state,
                          prior, compound_2CLJ, initial_position=[1, 1, 1])
        self.assertRaises(TypeError, mcmc_simulator.set_initial_state, prior,
                          compound_2CLJ, initial_position=[1, 1, 1, 'text'])
    ...
    def test_logp_output(self):
        mcmc_simulator, compound_2CLJ, prior = TestSetInitialState.setup()
        self.assertRaises(ValueError, mcmc_simulator.set_initial_state, prior,
                          compound_2CLJ, initial_position=[-1, -1, -1, -1])
...
class TestParameterProposal(unittest.TestCase):
    ...
    def test_sanity_check(self):
        ...
        new_params, proposed_log_prob = \
            mcmc_simulator.parameter_proposal(prior,
                                              proposed_params, compound_2CLJ)
...
class TestMCMCSteps(unittest.TestCase):
    def setup():
        ...
        mcmc_simulator.percent_dev_trace = \
            [mcmc_simulator.initial_percent_deviation]
    ...
    def test_outputs(self):
        ...
        for i in range(100):
            ...
            new_params, new_logp, acceptance = \
                mcmc_simulator.MCMC_Steps(prior, compound_2CLJ)
            self.assertEqual(move_proposals+1, mcmc_simulator.move_proposals)
            self.assertIs(bool, type(acceptance))

            if acceptance is True:
                self.assertEqual(move_acceptances+1,
                                 mcmc_simulator.move_acceptances)
                self.assertNotEqual(sum(new_params),
                                    sum(mcmc_simulator.current_params))
                self.assertNotEqual(new_logp,
                                    mcmc_simulator.current_log_prob)

            if acceptance is False:
                self.assertEqual(move_acceptances,
                                 mcmc_simulator.move_acceptances)
                self.assertEqual(new_params.all(),
                                 mcmc_simulator.current_params.all())
...
class TestWriteOutput(unittest.TestCase):
    ...
    def test_bad_input(self):
        ...
        self.assertRaises(TypeError,
                          mcmc_simulator.write_output, 'wrong_prior')
        self.assertRaises(TypeError, mcmc_simulator.write_output,
                          simulation_params['priors'], tag=1)
        self.assertRaises(TypeError, mcmc_simulator.write_output,
                          simulation_params['priors'], save_traj='int')
    ...
    def test_output_created(self):
        ...
        self.assertTrue(os.path.isfile(path+'/figures/' +
                                       'triangle_plot_params.png'))
        self.assertTrue(os.path.isfile(path+'/figures/' +
                                       'triangle_plot_percent_dev_trace.png'))
        ...
        self.assertTrue(os.path.isfile(path+'/trace/' +
                                       'percent_dev_trace_tuned.npy'))
        shutil.rmtree(path)
...
class TestFindMaxima(unittest.TestCase):
    ...
    def test_input(self):
        ...
        self.assertRaises(TypeError,mcmc_simulator.find_maxima,
                          'not_array',
                          compound_2CLJ)
